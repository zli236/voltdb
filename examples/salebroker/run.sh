...
function producecsv() {
    mkdir -p csv
    srccompile
    java -classpath obj:$CLIENTCLASSPATH:obj salebroker.produceCSV "$(pwd)/csv/"
}

# run the voltdb server locally
function loadcsv() {
    if [ ! -d "$(pwd)/csv/" ]; then producecsv; fi
    $CSVLoader -f csv/sellers.csv sellers
    $CSVLoader -f csv/items.csv items
    $CSVLoader -f csv/categories.csv categories
}
